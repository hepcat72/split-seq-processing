...
sub addInfileOption
  {
    ...
    push(@$usage_file_indexes,
	 addToUsage($get_opt_str,$flags,$smry_desc,$detail_desc,$required,
		    ...
		    $input_files_array->[$file_type_index],undef,$primary,
...
sub getOutdirFlag
  {
    ...
    my $flag = (defined($outdir_flag_array->[0]) ? $outdir_flag_array->[0] :
		getDefaultFlag(scalar(getOptStrFlags($default_outdir_opt))));
...
sub addOutfileSuffixOption
  {
    ...
    if(!defined($default) && $primary && !$required)
      {$default = 'STDOUT'}
...
sub addOption
  {
    ...
    my $internal_flglss = $in[12];#For internal use only
...
sub addArrayOption
  {
    ...
    elsif(scalar(@$get_opt_ref) && !defined($def_str))
      {$def_str = join(',',map {defined($_) ? $_ : 'undef'} @$get_opt_ref)}
...
#Globals used: $general_flag_hash
sub isOptRefUnique
  {
    my $get_opt_ref = $_[0];
    my $new_flag    = $_[1];
    my $uniq = !exists($general_flag_hash->{$get_opt_ref});
    if(!$uniq)
      {error("Same reference passed for options: ",
	     "[$general_flag_hash->{$get_opt_ref}] and [$new_flag].")}
    return(!exists($general_flag_hash->{$get_opt_ref}));
  }
...
sub add2DArrayOption
  {
    ...
    if(defined($get_opt_ref) &&
       ref($get_opt_ref) eq 'SCALAR' && isTwoDArray($$get_opt_ref))
      {$sub = sub {push(@{$$get_opt_ref},[split(/$delimiter/,$_[1])])}}
    elsif(defined($get_opt_ref) &&
...
sub addToUsage
  {
    ...
    #For access to option references whose values are controlled via subroutines
    #supplied to Getopt::Long, such as 1D & 2D arrays, infiles, outfiles,
    #outdirs, and suffixes
    my $varref      = $_[11];
    ...
    push(@$usage_array,{GETOPTKEY => $getoptkey,
			GETOPTREF => $varref,   #For internal use only
			OPTFLAG   => $flags_array,
...
sub determineRunMode
  {
    ...
    #found to be missing or not
    my $missing_req_default_infiles = (defined($_[1]) ? $_[1] : 0);
    ...
    my $status    = ($determine_default_run_mode ? getRunModeStatus() :
		     doUnsetRequiredOptionsExist($missing_req_default_infiles));
	...
	if($status == 0) #No required opts or all have values - proceed
	...
	else #Required opts not supplied
	  {
	    ...
	    #option do not exist
	    if($missing_req_default_infiles)
	      {$run = 1}
...
sub doUnsetRequiredOptionsExist
  {
    my $missing_req_default_infiles = $_[0];
...
sub processCommandLine
  {
    ...
    #Determine the programmer's default run mode (or user-explicit run mode)
    determineRunMode(1);
    ...
    my $fatal_def_req_file_errors = [loadDefaults()];

    #Determine the user's default run mode
    ...
    #If we are in either run or dry run mode, then any issues with default files
    #or directories must now cause a fatal error
    if(scalar(@$fatal_def_req_file_errors))
      {
...
sub loadDefaults
  {
    ...
    #values)
    my @def_fatal_file_errors = loadDefaultFiles();
    loadDefaultOutdirs();

    return(@def_fatal_file_errors);
  }
...
sub loadDefaultFiles
  {
		...
		foreach my $globstr (@$flag_instance_array)
		  {
		    ...
		    if(scalar(@$newfiles))
		      {
			#If this file type is required, record that we got one
			$got_some = 1;
			push(@$files,@$newfiles);
		      }
		    else
	...
	if($got_some)
	  {
	    $errmsg =
	      join('',
		   ("Some of the expected default input files were not found: ",
	...
	#If one of the file types is a required option
	elsif($some_required)
	  {
	    $errmsg =
	      join('',
		   ("Expected required default input files were not found: ",
		    "[$all_err_globs].  Unable to proceed.  Please create the ",
		    "default files."));
	    push(@$fatal_errors,$errmsg);
	  }
	...
	if(defined($quiet))
	  {
	    ...
	    if($cleanup_mode == 2 && defined($DEBUG) && !isDebug())
	      {return(0)}
...
sub flushStderrBuffer
  {
	...
	if($type eq 'verbose')
	...
	elsif($type eq 'error' || $type eq 'warning')
	  {
	    ...
	    #If there's already been a fatal error, do not print warnings and
	    #only print the first error unless in debug mode.  This assumes the
	    #first error was a fatal error and that all subsequent errors were
	    #during the cleanup
	    ##TODO: Save the cleanup_mode state with each error and warning so I
	    ##      don't have to assume the first error is the fatal error.
	    ##      See requirement #339
	    if(!defined($cleanup_mode) || $cleanup_mode != 2 ||
	       !defined($DEBUG) || $DEBUG ||
	       ($cleanup_mode == 2 && $tmp_msg =~ /^ERROR1:/))
...
sub hasLinkedSuffix
  {
    ...
    return(defined($outfile_suffix_array->[$usghash->{FILEID}]) &&
	   scalar(@{$outfile_suffix_array->[$usghash->{FILEID}]}));
  }
...
sub showDefault
  {
    ...
    if($type ne 'bool' && $type ne 'negbool' && $type ne 'count')
      ...
      {return((defined($default) && $default != 0) ||
	      (ref($ref) eq 'SCALAR' && defined($$ref) && $$ref != 0) ||
	      #Apparently count types handle the reference differently
	      (ref($ref) eq '' && defined($ref) && $ref != 0))}
...
sub displayDefault
  {
    ...
    if(showDefault($usage_hash->{OPTTYPE},
                   getVarRef($usage_hash),
                   $usage_hash->{DEFAULT}))
      {
	#The "real" default is the VARREF's value.  It should only be shown if a
	#display default was not provided
	my $dispdef     = $usage_hash->{DEFAULT};
	my $realdef     = getVarRef($usage_hash);
	my $showdispdef = defined($dispdef);
	my $showrealdef = 0;
        my $undef       = '';   #Don't show anything if default not defined
        my $emptystr    = '[]';
	...
	if(!$showdispdef)
	  {
	    $showrealdef = 1;
            ...
            #There's no default if it's a scalar referent & its value is undef
	    if(ref($realdef) eq 'SCALAR')
	      {
		if(defined($$realdef))
		  {$realdef = $$realdef}
		else
	    ...
	    #Display default should have been set automatically.  If we get
	    #here, it means the default value was empty, i.e. the array was not
	    #pre-populated.
	    elsif($usage_hash->{OPTTYPE} =~ /array/)
	      {$showrealdef = 0}
	    elsif(defined($realdef) && ref($realdef) ne 'SCALAR')
	      {$realdef = 'unable to display default'}
	    elsif(!defined($realdef))
	      {$showrealdef = 0}
	  }

	#If there's a default to display
        if($showdispdef || $showrealdef)
          {
	    my $def = ($showdispdef ? $dispdef : $realdef);
            if($def eq '')
              {
                if(($showdispdef && defined($dispdef)) ||
		   ($showrealdef && defined($realdef)))
                  {$disp = $emptystr}
                else
                  {$disp = $undef}
              }
              ...
              {
		#Do not display count type defaults when 0
		if($usage_hash->{OPTTYPE} eq 'count' && $def == 0)
		  {$disp = $undef}
		else
		  {$disp = "[$def]"}
	      }
...
sub hasDefinedDisplayDefault
  {
    my $usage_hash = $_[0];
    return(defined($usage_hash->{DEFAULT}) && $usage_hash->{DEFAULT} ne '');
  }

sub hasDefinedActualDefault
  {
    my $usage_hash = $_[0];
    my $varref     = getVarRef($usage_hash);

    if(ref($varref) eq 'CODE')
      {
	debug("Unable to determine default value for option [",
	      getDefaultFlag($usage_hash->{OPTFLAG}),"].");
	return(0);
      }
    ...
    if($usage_hash->{OPTTYPE} eq 'string'  ||
      ...
      {return(defined($varref) && scalar(@$varref))}
...
#Takes a usage hash for a particular option and returns a reference to the
#variable that is populated by that option.  It does not return CODE references
#created by this module but it will for CODE refs supplied by the programmer
#using this module
#Be careful not to modify any values referenced by the returned reference!!!
sub getVarRef
  {
    my $usage_hash = $_[0];
    ...
    if(scalar(grep {$usage_hash->{OPTTYPE} eq $_}
      ...
      {return($GetOptHash->{$usage_hash->{GETOPTKEY}})}
